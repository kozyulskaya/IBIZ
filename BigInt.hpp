...
class BigInt {
    ...
    public:
        ...
        static BigInt big_random(size_t num_digits);
};
...
std::mt19937 big_generator(rand_generator());
std::uniform_int_distribution<int> big_rnd_int_distrib(0, INT_MAX);
BigInt BigInt::big_random(size_t num_digits = 0) {
    if (num_digits == 0)    // the number of digits were not specified
        ...
        num_digits = 1 + big_rnd_int_distrib(big_generator) % MAX_RANDOM_LENGTH;

    BigInt big_rand;
    big_rand.value = "";    // clear value to append digits

    // ensure that the first digit is non-zero
    big_rand.value += std::to_string(1 + big_rnd_int_distrib(big_generator) % 9);

    while (big_rand.value.size() < num_digits)
        big_rand.value += std::to_string(big_rnd_int_distrib(big_generator));
